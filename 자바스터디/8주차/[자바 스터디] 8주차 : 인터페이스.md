...
## 추가 정리할 것
1. 인터페이스의 개선
   1. 스프링 프레임워크의 코드들도 `Interface -> Abstract Class (Default 메소드용) -> 구현체` 에서 `Interface(with Default 메소드) -> 구현체`로 개선되고있다
2. `default` 메소드의 사용 예시로, `HandlerIntercepter`의 경우 모든 메소드를 사용할 필요가 없을 경우를 고려해 본문이 비어있는 `default` 메소드로 api를 정의하고 있음 -> 아니면 모든 메소드를 구현해야 하므로!
   1. 즉, 기능 추가 시 뿐만 아니라 최초 구현시에도 모든 구현체에서 사용하고 싶지 않게 하려면 `default` 메소드를 사용할 수도 있다
3. constant interface가 안티패턴인 이유
4. default 다중상속 시 컴파일 에러 관련하여 -> 하나가 static이면 문제 X
   1. 어차피 다른 영역의 메소드 이므로 재정의 / 호출되는 것은 default 뿐
5. 강한 결합 vs 느슨한 결합 (그림)
   1. 인터페이스를 사용하는 근본적인 이유
